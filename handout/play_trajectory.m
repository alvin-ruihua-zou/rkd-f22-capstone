...
% CONSTANT VELOCITY
traj = trajectory_const_vel(trajectory(:,1:end), times, frequency);
traj_vel = diff(traj,1,2);

for i = 1 : (size(traj, 2) - 1)
    % Send command to the robot
    cmd.position = traj(:,i)'; % transpose turns column into row vector for commands
    if (use_velocity)
...
traj = trajectory_trap_vel(trajectory(:,1:end), times, frequency, 0.2);
traj_vel = diff(traj,1,2);
...
%{
% SPLINE
traj = trajectory_spline(trajectory(:,1:end), times, frequency);
traj_vel = diff(traj,1,2);
